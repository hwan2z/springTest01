...
public class BeanDomain {
	...
	@Min(value=18, message="최소 값은 18이어야 합니다.")
	@Max(value=50, message="최대 값은 50 이상이 될 수 없습니다.")
	private int age;
	
	@NotNull(message="이름을 입력해주십시오.")
	private String name;
	
	@DecimalMax(value="99999.999", message="숫자가 99999.999 이상이 될 수 없습니다.")
	@DecimalMin(value="1.00", message="숫자 값은 1.00 보다 작을 수 없습니다.")
	private float amount = 0f;
	
	@NotNull
	@Pattern(regexp="^\\d{6}", message="우편번호는 6자리이어야 합니다.")
	...
	@Email(message = "이메일을 잘못 입력하였습니다.")
	private String email;
	
	@Length(max=50, min=10, message="메시지는 10문자이상 50문자까지 입력할 수 있습니다.")
	private String description;
	
	@NotBlank(message="ID는 반드시 입력해야 합니다.")
	//String 타입에만 적용할 수 있음
	private String id;
	
	@NotEmpty(message="제품 목록이 있어야 합니다.")
